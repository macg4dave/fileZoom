...
fn settings_keyboard_interaction() {
    ...
    let idx = labels.iter().position(|&s| s == "Settings").expect("Settings label present");
    app.menu_index = idx;
    app.menu_activate();
    // should be in Settings mode
    match &app.mode {
        fileZoom::app::Mode::Settings { selected } => { assert_eq!(*selected, 0); }
    ...
    handlers::handle_key(&mut app, KeyCode::Enter, 10).unwrap();
    match &app.mode {
        fileZoom::app::Mode::Message { title, .. } => { assert_eq!(title, "Settings Saved"); }
...
fn settings_mouse_click_toggle_and_save() {
    ...
    let me = fileZoom::input::mouse::MouseEvent { column: rect.x + 2, row: rect.y + 1, kind: fileZoom::input::mouse::MouseEventKind::Down(fileZoom::input::mouse::MouseButton::Left) };
    let _ = handle_mouse(&mut app, me, area).unwrap();
    assert_eq!(app.settings.mouse_enabled, false);

    // Click Save (footer left half)
    let footer_row = rect.y + rect.height.saturating_sub(2);
    let me2 = fileZoom::input::mouse::MouseEvent { column: rect.x + 1, row: footer_row, kind: fileZoom::input::mouse::MouseEventKind::Down(fileZoom::input::mouse::MouseButton::Left) };
