...
fn handle_normal(app: &mut App, code: KeyCode, page_size: usize) -> anyhow::Result<bool> {
    match code {
        ...
        KeyCode::Tab => {
            app.active = match app.active { Side::Left => Side::Right, Side::Right => Side::Left };
        }
        ...
        KeyCode::End => match app.active {
            Side::Left => { if !app.left.entries.is_empty() { app.left.selected = app.left.entries.len() - 1 } }
            Side::Right => { if !app.right.entries.is_empty() { app.right.selected = app.right.entries.len() - 1 } }
        },
