...
pub fn draw_list<B: Backend>(f: &mut Frame<B>, area: Rect, panel: &Panel, active: bool) {
    ...
    let visible = if panel.entries.len() > panel.offset { &panel.entries[panel.offset..std::cmp::min(panel.offset + list_height, panel.entries.len())] } else { &[] };
...
pub fn draw_preview<B: Backend>(f: &mut Frame<B>, area: Rect, panel: &Panel) {
    ...
    let visible = if panel.preview_offset < lines.len() { &lines[panel.preview_offset..std::cmp::min(panel.preview_offset + max_lines, lines.len())] } else { &[] };
